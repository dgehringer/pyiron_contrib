...
class DAMASKjob(GenericJob):
    """
    The job provides an interface to the [damask project]( https://damask.mpie.de) 

    Input:
        job.material "Material configuration : It contains the five parts of (a.) homogenization, (b.) microstructure, (c.) crystallite, (d.) phase, (e.) texture"
        
        job.load "Load defination : It contains information about applied stress and strain."

        job.geometry "Geometry : It describes volume elements on a grid"

    Output:
        job.stress, job.strain ---> It contains the stress, strain values per time increment
        
    Example:
        >>> job = pr.create_job(DAMASKjob, job_name='demo')
        
        >>> job.material = material # material_configuration
        >>> job.load = load # boundary condition, load_file
        >>> job.geometry = geometry # geometry
        or 
        >>> job.path = path(it assumes that material, load and geometry files are in the current working directory)
